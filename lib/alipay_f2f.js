...
-----END PUBLIC KEY-----`;
	...
	/**
	 * 验证callback签名
	 * @param  {object} postData callback post数据对象
	 * @return {Promise}   是否通过验证
	 */
	verifyCallback(postData) {
		return new Promise((resolve, reject) => {
			var notifyId = postData["notify_id"];
			var sellerId = postData["seller_id"];
			if(notifyId == undefined || notifyId == "" || notifyId == null) {
				return resolve(false);
			}
			if(sellerId == undefined || sellerId == "" || sellerId == null) {
				return resolve(false);
			}
			aop.notifyVerify(notifyId, sellerId, this._config).then(resolve).catch(reject);
		});
	}
